...
if (!empty($item)): ?>
    <?php if (!empty(SHOW_ALL_PRICES_CATALOG) && !empty($item['prices']) && is_array($item['prices'])): ?>
        <?php foreach ($item['prices'] as $price): ?>
            ...
            <?php if (count($user['price_types']) > 1 || (defined(SHOW_PRICE_TYPES))): ?>
            ...
            <?php if (!empty($price['discount'])): ?>
                <div class="product-item-oldprice <?= $price['price_type_id'] !== $user->priceTypeId ? 'inactive' : '' ?>">
                    ...
                    <span class="product-item-currency"><?= $price['currency_sign'] ?></span><span class="product-item-measure">/<?= $item['unit_sign'] ?></span>
                    <div class="product-item-priceline"></div>
                </div>
            <?php endif; ?>

            <div class="product-item-price <?= $price['price_type_id'] !== $user->priceTypeId ? 'inactive' : '' ?>">
    ...
    <?php else: ?>
        ...
        <?php if (!empty($item['discount'])): ?>
            <div class="product-item-oldprice">
                ...
                <span class="product-item-currency"><?= $item['currency_sign'] ?></span><span class="product-item-measure">/<?= $item['unit_sign'] ?></span>
                <div class="product-item-priceline"></div>
            </div>
        <?php endif; ?>
